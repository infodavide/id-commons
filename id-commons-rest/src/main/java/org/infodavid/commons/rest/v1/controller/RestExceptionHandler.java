...
import org.infodavid.commons.rest.exception.TooManyRequestsException;
...
public class RestExceptionHandler {
    ...
    public final ResponseEntity<Object> handleException(final Exception e, final WebRequest request) throws Exception {
        if (e instanceof ConversionFailedException) {
            LOGGER.debug(String.format("A conversion error occurred on: %s, %s", getRequestUri(request), ExceptionUtils.getRootCause(e).getMessage())); // NOSONAR Always written

            return new ResponseEntity<>(e.getMessage(), HttpStatus.BAD_REQUEST);
        }

        LOGGER.error(String.format("An error occurred on: %s", getRequestUri(request)), e); // NOSONAR Always written
